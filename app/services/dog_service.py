...
class DogService:
    ...
    def update(self, dog_id: int, dog_update: DogUpdateRequest) -> str:
        try:
    ...
    def update_status(self, dog_id: int, dog_status_update: DogStatusUpdateRequest) -> DogResponse:
        dog = self._dao.get_by_id(dog_id)
        if not dog:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Dog not found.")
        ...
        return {
            "message": f"Dog status updated to '{new_status.value}'.",
        }

    def deactivate(self, dog_id: int) -> DogResponse:
        dog = self._dao.deactivate(dog_id)
        if not dog:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Dog not found.")
        return {
            "message": "Dog deactivated. Dogs are never permanently deleted.",
        }
